...
// #include <linux/gpio/gpio-consumer.h>
#include <sound/soc.h>
...
struct ak7755_priv {
	...
	struct gpio_desc *pdn_gpio, *power_gpio, *amp_gpio;
...
static int ak7755_hw_set_clk(struct snd_soc_component *component, int nfs)
{
	u8 	fs;
	...
	if (aec == 1){
		snd_soc_component_update_bits(component, AK7755_CE_POWER_MANAGEMENT, 0x80, 0x0);		//If aec is enable, only CH-L enable by DSP
	}
		...
		struct snd_soc_dai *dai)
{
	...
	ak7755_hw_set_clk(component, params_rate(params));

	ret = snd_soc_component_update_bits(component, AK7755_C1_CLOCK_SETTING2, 0x30, (bickfs << 4));
...
static int ak7755_set_status(enum ak7755_status status)
{
	...
	case RUN:
		...
		mdelay(10);
		snd_soc_component_update_bits(ak7755_data->component, AK7755_CF_RESET_POWER_SETTING, 0xd, 0xc);  // CRESETN bit = DSPRESETN = 1;
		mdelay(10);
		break;
	case DOWNLOAD:
...
static int ak7755_ram_download(const u8 *tx_ram, u64 num, u16 crc)
{
	...
	if (rc < 0) {
		...
		if ( fast_boot == 1 ) {
			...
			snd_soc_component_update_bits(ak7755_data->component, AK7755_CF_RESET_POWER_SETTING, 0x0d,0x0c);  // system rest release
		}
		return rc;
	}
...
static int ak7755_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *component_dai)
{
	...
	if(gpiod_get_value(ak7755->amp_gpio) > 0) {
		switch (cmd) {
		case SNDRV_PCM_TRIGGER_START:
		case SNDRV_PCM_TRIGGER_RESUME:
		case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
				gpiod_set_value(ak7755->amp_gpio, 0);
			}
			break;
		...
		case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK){
				gpiod_set_value(ak7755->amp_gpio, 1);
			}
			break;
		default:
			break;
...
static int ak7755_init_reg(struct snd_soc_component *component)
{
	...
	if ( gpiod_get_value(ak7755->power_gpio) > 0 ) {
		// gpiod_set_value(ak7755->power_gpio, ak7755->power_active);
		gpiod_set_value(ak7755->power_gpio, 0);
		msleep(2);		//wait stabilization
	}

	if ( gpiod_get_value(ak7755->pdn_gpio) > 0 ) {
		// gpiod_set_value(ak7755->pdn_gpio, ak7755->pdn_active);
		gpiod_set_value(ak7755->pdn_gpio, 0);
		msleep(5);		//wait stabilization
		// gpiod_set_value(ak7755->pdn_gpio, !ak7755->pdn_active);
		gpiod_set_value(ak7755->pdn_gpio, 1);
		msleep(5);
	}
	for (i=0;i<ARRAY_SIZE(ak7755_reg_defaults); i++)
		snd_soc_component_write(component, ak7755_reg_defaults[i].reg,  ak7755_reg_defaults[i].def);
	...
	if(dsp_bypass == 0) {
		...
		snd_soc_component_update_bits(component, AK7755_D2_MIC_GAIN_SETTING, 0x08, 0x03);		//set input gain 6dB
...
// 	ak7755->pdn_gpio = of_get_named_gpio(np, "ak7755,pdn-gpio", 0, &flags);
...
// 	ak7755->power_gpio = of_get_named_gpio(np, "ak7755,pwr-gpio", 0, &flags);
...
static int ak7755_parse_dt(struct ak7755_priv *ak7755)
{
	...
	/* 1. Request PDN GPIO */
	/* "ak7755,pdn-gpio" in DT maps to "ak7755,pdn" or "pdn" in devm_gpiod_get() */
	ak7755->pdn_gpio = devm_gpiod_get_optional(dev, "ak7755,pdn", GPIOD_OUT_LOW);
	if (IS_ERR(ak7755->pdn_gpio)) {
		dev_err(dev, "Failed to get ak7755 pdn-gpio\n");
		return PTR_ERR(ak7755->pdn_gpio);
	}

	/* 2. Request Power GPIO */
	ak7755->power_gpio = devm_gpiod_get_optional(dev, "ak7755,pwr", GPIOD_OUT_LOW);
	if (IS_ERR(ak7755->power_gpio)) {
		dev_err(dev, "Failed to get ak7755 pwr-gpio\n");
		return PTR_ERR(ak7755->power_gpio);
	}

	/* 3. Request Amp GPIO */
	ak7755->amp_gpio = devm_gpiod_get_optional(dev, "ak7755,amp", GPIOD_OUT_LOW);
	if (IS_ERR(ak7755->amp_gpio)) {
		dev_err(dev, "Failed to get ak7755 amp-gpio\n");
		return PTR_ERR(ak7755->amp_gpio);
	}
...
static int ak7755_probe(struct snd_soc_component *component)
{
	...
	if ( ret != 0 ) {
		dev_err(component->dev, "Failed to parse devict tree: %d\n", ret);
		return ret;
	}

	ak7755_init_reg(component);
	akdbgprt("\t[AK7755 init_ak7755_pd] %s(%d)\n",__FUNCTION__,__LINE__);
	ak7755_data->component = component;
...
static void ak7755_remove(struct snd_soc_component *component)
{
	...
	if ( gpiod_get_value(ak7755->pdn_gpio) > 0 ) {
		gpiod_set_value(ak7755->pdn_gpio, 0);
		gpio_free(desc_to_gpio(ak7755->pdn_gpio));
	}
...
static int ak7755_suspend(struct snd_soc_component *component)
{
	...
	for(i = 0; i < AK7755_MAX_REGNUM; i++) {
		ak7755_data->reg_cache[i] = snd_soc_component_read(component, i + 0xc0);
	}
	ak7755_data->reg_cache[1] &= 0xfe;		//Make sure  CKRESETN bit is reset

	ak7755_set_bias_level(component, SND_SOC_BIAS_STANDBY);

	if ( desc_to_gpio(ak7755->amp_gpio) > 0 ) {
		gpiod_set_value(ak7755->amp_gpio, 1);
	}
	if ( gpiod_get_value(ak7755->pdn_gpio) > 0 ) {
		gpiod_set_value(ak7755->pdn_gpio, 0);
	...
	if ( gpiod_get_value(ak7755->power_gpio) > 0 ) {
		gpiod_set_value(ak7755->power_gpio, 1);
	}
...
static int ak7755_resume(struct snd_soc_component *component)
{
	...
	snd_soc_component_write(component,AK7755_C0_CLOCK_SETTING1,0x08);
	snd_soc_component_write(component,AK7755_C1_CLOCK_SETTING2,0x00);
	for(i = 0; i < AK7755_MAX_REGNUM; i++) {
		snd_soc_component_write(component, i+0xc0, ak7755_data->reg_cache[i]);
	}

	snd_soc_component_update_bits(component, AK7755_C1_CLOCK_SETTING2, 0x01, 0x01);
	snd_soc_component_write(component,AK7755_CF_RESET_POWER_SETTING,0x01);
	snd_soc_component_write(component,AK7755_DA_MUTE_ADRC_ZEROCROSS_SET,0x10);
	mdelay(12);
	...
	snd_soc_component_write(component,AK7755_DA_MUTE_ADRC_ZEROCROSS_SET,0x00);
...
static int ak7755_spi_remove(struct spi_device *spi)
{
	if ( ak7755_data->amp_active > 0 ) {
		gpiod_set_value(ak7755_data->amp_gpio, !ak7755_data->amp_active);
		devm_gpio_free(&spi->dev,ak7755_data->amp_active);
	}
	if ( ak7755_data->pdn_gpio > 0 ) {
		gpiod_set_value(ak7755_data->pdn_gpio, ak7755_data->pdn_active);
		devm_gpio_free(&spi->dev,ak7755_data->pdn_gpio);
	}
	if ( ak7755_data->power_gpio > 0 ) {
		gpiod_set_value(ak7755_data->power_gpio, !ak7755_data->power_active);
